# ===============================================================
# NHANES 2007–2018: CARGA + LIMPIEZA + LABELS + FEATURES + SELECCIÓN
# ===============================================================
...
def load_nhanes_data(data_dir='./data', cycles=None):
    ...
    for cycle in cycles:
        ...
        demo['CYCLE'] = cycle
        cycle_data = demo
        print(f"  ✓ DEMO cargado: {len(demo):,} registros, {len(demo.columns)} columnas")
        ...
        if exam_file.exists():
            ...
            if 'SEQN' in exam.columns:
                ...
                print(f"  ✓ EXAM merged: {len(exam):,} registros, {len(exam.columns)} columnas")
        ...
        if lab_file.exists():
            ...
            if 'SEQN' in lab.columns:
                ...
                print(f"  ✓ LAB merged: {len(lab):,} registros (solo para label)")
        ...
        if quest_file.exists():
            ...
            if 'SEQN' in quest.columns:
                ...
                print(f"  ✓ QUEST merged: {len(quest):,} registros, {len(quest.columns)} columnas")
...
print(f"\n🚨 COLUMNAS LAB detectadas: {len(LAB_COLUMNS)}")
with open('LAB_COLUMNS_FORBIDDEN.txt','w',encoding='utf-8') as f:
    f.write('\n'.join(LAB_COLUMNS))

# ===============================================================
# LABELS (DIABETES / HIPERTENSIÓN)
# ===============================================================
...
def create_hypertension_label(df):
    ...
    mask = (df['sbp_mean']>=130)|(df['dbp_mean']>=80)
    df.loc[mask,'label_hypertension']=1
    df=df[has_bp].copy()
    prevalence=df['label_hypertension'].mean()
    print(f"✅ Label Hipertensión ({len(df):,}, prev. {prevalence:.1%})")
    return df
...
def select_features_no_leakage(df, target='label_diabetes'):
    candidate_features = [
        ...
        'cigarettes_per_day','current_smoker','ever_smoker','total_active_days','meets_activity_guidelines',
...
X_train, y_train, features = select_features_no_leakage(df_train, target='label_diabetes')
